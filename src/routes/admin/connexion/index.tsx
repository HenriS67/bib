...
export const useLogin = routeAction$(async (credentials, event) => {
  ...
  if (user?.active && verifyPassword(String(credentials.password || ""), user.passwordHash)) {
    event.cookie.delete(ADMIN_COOKIE, { path: "/admin" });
...
export default component$(() => {
  ...
  const submitLogin = $(async (event: SubmitEvent, form: HTMLFormElement) => {
    event.preventDefault();
    const result = await login.submit(Object.fromEntries(new FormData(form)));
    if (result.value && "authenticated" in result.value && result.value.authenticated) {
      window.location.assign("/admin/auteurs");
    }
  });
  return (
    <main class="flex min-h-screen items-center justify-center bg-[#f4f1ea] px-5 text-[#1d211d]">
      <div class="w-full max-w-md border border-[#1d211d] bg-[#fbfaf6] p-7 md:p-10">
        ...
        <form preventdefault:submit onSubmit$={submitLogin} class="space-y-5">
